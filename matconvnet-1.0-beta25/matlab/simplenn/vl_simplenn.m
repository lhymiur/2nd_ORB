...
for i=1:n
  ...
  switch l.type    
    ...
    case 'conv'
      if l.stride == 1  
        ...
        res(i+1).x = vl_nnconv(res(i).x, l.weights{1}, l.weights{2}, ...
            ...
            l.opts{:}, ...
            cudnn{:}) ;
        res(i+1).x = res(i+1).x(l.stride:l.stride:end,l.stride:l.stride:end,:);
    ...
    case 'mean_filling'
        [w,h,~] = size(res(i).x);
        res(i+1).x = repmat(mean(mean(res(i).x,1),2),[w,h]);
        
   case 'global_mean'
        res(i+1).x = mean(mean(res(i).x,1),2);   
        
    
    case 'cross_layer_add'
        res(i+1).x = res(i).x+res(i-l.stride).x;
        
    case 'cross_layer_multi_add'
        temp = 0;
        for ii=1:length(l.stride)
            temp = temp + l.weights(ii)*res(i-l.stride(ii)).x;
        end
        res(i+1).x = temp;
        %res(i+1).x = 0.5*res(i).x+1*res(i-6).x+1*res(i-9).x;

        
    case 'cross_layer_sub'
        res(i+1).x = res(i-l.stride).x-res(i).x;
    
    case 'cross_layer_multiplication' 
        res(i+1).x = res(i).x.*res(i-l.stride).x;
